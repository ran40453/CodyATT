"""
updateJsonfromFIT.py — Convert company attendance Excel (.xlsx) to JSON.

Usage:
  GUI mode (default):
    python updateJsonfromFIT.py

  CLI mode (for CI / GitHub Actions):
    python updateJsonfromFIT.py --excel path/to/file.xlsx --json path/to/app.json --output output.json
"""
...
def generate_final_json(excel_path, json_path, save_path):
    """Core logic — accepts file paths directly (no GUI)."""

    if not excel_path or not json_path:
        print("❌ 未提供 Excel 或 JSON 路徑，程式終止。")
        return
    ...
    for m in range(1, 13):
        ...
        if s_attn not in wb.sheetnames:
            continue
    ...
    if save_path:
        ...
        print(f"\n✅ 轉換成功！產出 {len(all_output)} 筆記錄 → {save_path}")
    else:
        print("❌ 未指定輸出路徑")


def gui_mode():
    """Original GUI mode using tkinter file dialogs."""
    ...
    generate_final_json(excel_path, json_path, save_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert FIT Excel attendance to JSON")
    parser.add_argument("--excel", help="Path to company Excel (.xlsx) file")
    parser.add_argument("--json", help="Path to app JSON reference file")
    ...
    if args.excel and args.json:
        # CLI mode
        output = args.output or f"app_synced_{os.path.basename(args.excel).replace('.xlsx', '')}.json"
        generate_final_json(args.excel, args.json, output)
    else:
        # GUI mode (legacy)
        gui_mode()