;(function (global) {
  ...
  async function saveToSheet() {
    ...
    if (isExternalHost) {
      console.warn(
        '[OT] saveToSheet 僅支援在 Apps Script Web App 內使用，外部網域將略過寫回。'
      );
      alert(
        '目前這個版本只當作前端試算使用，要寫回 Google Sheet 請從 Apps Script 網頁版本開啟。'
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        '[OT] 無法直接 POST 寫回，請於 Apps Script 端提供 doPost/save handler 或改用 /exec?ui=1 啟動 HtmlService：',
        e
      );
      throw e;
    }
  ...
  const SHEET_URL =
    'https://script.google.com/macros/s/AKfycbybvyOVF_Qj8C9FQ4QaKj1hAmp7tsspkdNR1IlPDBpuNbakKy4GpuhZuygxrPiYDgMv2Q/exec?api=1';
